...
namespace Hangfire.JobDomains.Loader {
	...
	public class PluginHost : IDisposable {
        ...
        public Sponsor<object> GetInstance(string assemblyName, string typeName, object[] paramers)
        {
            object instance = null;
            try
            {
                instance = Loader.GetInstance(assemblyName, typeName, paramers);
            }
            catch
            {
                return null;
            ...
            if (instance != null)
                return new Sponsor<object>(instance);
            else
                return null;
        }
        ...
        /// <returns></returns>
        public Sponsor<TInterface> GetImplementation<TInterface>(object[] paramers) where TInterface : class {

            TInterface instance = default(TInterface);
            instance = Loader.GetImplementation<TInterface>(paramers);
        ...
        ///// <returns></returns>
        public IEnumerable<Sponsor<TInterface>> GetImplementations<TInterface>(object[] paramers) where TInterface : class
        {
            LinkedList<Sponsor<TInterface>> instances = new LinkedList<Sponsor<TInterface>>();

            foreach (TInterface instance in Loader.GetImplementations<TInterface>(paramers))
            {
        ...
        {
            var _AssemblyPath = typeof(PluginHost).Assembly.Location;
            if (_AssemblyPath.Contains("Temporary ASP.NET Files") == false) return _AssemblyPath;
            string str =   Path.GetDirectoryName(_AssemblyPath) + @"\__AssemblyInfo__.ini";
            str = File.ReadAllText(str, System.Text.Encoding.Unicode);
            int index = str.IndexOf("file:///") + 8;
            int length = str.LastIndexOf("/");
            str = str.Substring(index, length - index + 1);
            return str;
        }
